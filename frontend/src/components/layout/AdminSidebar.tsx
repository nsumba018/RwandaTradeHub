...
import {
  LayoutDashboard, FileCheck, CreditCard, Users, Building2,
  ArrowLeftRight, Bell, Settings, LogOut,
...
const navItems = [
  ...
  { to: '/admin/verification',  icon: FileCheck,        label: 'Invoice Verification' },
  { to: '/admin/financing',     icon: CreditCard,       label: 'Financing Requests' },
  { to: '/admin/investors',     icon: Users,            label: 'Investors' },
  { to: '/admin/smes',          icon: Building2,        label: 'SMEs' },
  ...
  return (
    ...
    >
        ...
        <AnimatePresence>
          {!collapsed && (
            ...
            >
              ...
              <p className="text-[#64748B] text-xs">Hub · Admin</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="px-2 py-4 border-t border-white/5 space-y-0.5">
        ...
        <button
          onClick={onToggle}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-[#475569] hover:bg-white/5 hover:text-white transition-all duration-150"
        >
